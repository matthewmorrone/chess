...
_.mixin({
	alphaRange: function(start, stop) {
		...
		return _.map(_.range(start, stop), function(i) {
			return alpha[i]
		});
	},
...
chess.boardTemplate = "<table><% " + "_.each(_.range(8, 0, -1), function(i) { " + "%> <tr> <%" + "_.each(_.alphaRange(0, 8), function(a) { " + "var piece = chess.get(a+i); %>" + "<td class='square <%= chess.square_color(a+i) %>' data-square='<%= a+i %>' > <%" + "if (piece) { %> " + "<span class='piece' data-color='<%= piece.color %>' data-type='<%= piece.type %>' >" + "<%= chess.symbols[piece.color][piece.type] %>" + "</span> <% " + "} %>" + "</td><% " + "}); " + "%> </tr> <%" + "}); %>" + "</table>";
